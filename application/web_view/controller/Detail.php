...
class Detail extends Controller
{
    ...
    public function index($class_id)
    {
        ...
        // 班级照片
        $class_photos = Db::name('photos')->where('class_id', $class_id)->select();
        if($class_photos){
        ...
        // 名师风采
        $hasTeacher = Db::name('teachers')->where('class_id',$class_id)->select();
        $teacher = Db::name('teachers')->where('class_id', $class_id)->paginate(3, true,['var_page' => 't_page']);
        if($hasTeacher){
